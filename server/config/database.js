...
async function seedInitialData() {
  try {
    ...
    const [buildResult] = await pool.execute(`
      INSERT INTO game_builds (version, title, description, file_size, test_instructions, known_issues, uploaded_by)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [
      '0.1.0',
    ...
    await pool.execute(`
      INSERT INTO announcements (title, content, author_id, is_sticky, target_roles)
      VALUES (?, ?, ?, ?, ?)
    `, [

...
const MuseFuzeFinances: React.FC = () => {
  ...
  const fetchFinancialData = async () => {
    try {
      ...
      // If no preferences in sessionStorage, check localStorage (only if consent was given)
      const consent = localStorage.getItem('musefuze-cookie-consent');
      
      if (consent === 'accepted') {
        try {
          const storedPreferences = localStorage.getItem('musefuze-cookie-preferences');
...
====================================================`;
  ...
  const resetToDefaults = () => {
    ...
    setHasConsented(false);
    setShowConsent(true);
    sessionStorage.removeItem('musefuze-cookie-preferences');
    localStorage.removeItem('musefuze-cookie-consent');
    localStorage.removeItem('musefuze-cookie-preferences');
  };
  ...
  // Accept all cookies
    ...
    localStorage.setItem('musefuze-cookie-preferences', JSON.stringify(allAccepted));
    sessionStorage.setItem('musefuze-cookie-preferences', JSON.stringify(allAccepted));
  ...
  // Decline all optional cookies
    ...
    setPreferences(essentialOnly);
                <button
    // Store in sessionStorage only (not localStorage) since user declined persistent cookies
    sessionStorage.setItem('musefuze-cookie-preferences', JSON.stringify(essentialOnly));
    localStorage.removeItem('musefuze-cookie-preferences');
                  onClick={() => setShowTaxReportModal(false)}
                  className="px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors"
    setShowPreferences(false);
  ...
  const savePreferences = (newPreferences: CookiePreferences) => {
    ...
    // If any non-essential cookies are enabled, consider this as consent
    const hasNonEssentialCookies = 
      updatedPreferences.functional || 
      updatedPreferences.analytics || 
      updatedPreferences.marketing;
    
    setHasConsented(hasNonEssentialCookies);
    
    // Save to storage
    localStorage.setItem('musefuze-cookie-consent', hasNonEssentialCookies ? 'accepted' : 'declined');
    
    if (hasNonEssentialCookies) {
      // If user accepted some cookies, store in both storages
      localStorage.setItem('musefuze-cookie-preferences', JSON.stringify(updatedPreferences));
      sessionStorage.setItem('musefuze-cookie-preferences', JSON.stringify(updatedPreferences));
    } else {
      // If user declined all optional cookies, only store in sessionStorage
      sessionStorage.setItem('musefuze-cookie-preferences', JSON.stringify(updatedPreferences));
      localStorage.removeItem('musefuze-cookie-preferences');
    }
